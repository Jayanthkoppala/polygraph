...
class RetroEffectImpl extends Effect {
  private readonly retroUniforms: Map<string, THREE.Uniform<number>>;
  ...
  set colorNum(v: number) {
    const uniform = this.retroUniforms.get('colorNum');
    if (uniform) uniform.value = v;
  }
  get colorNum() {
    return this.retroUniforms.get('colorNum')?.value as number;
  }
  set pixelSize(v: number) {
    const uniform = this.retroUniforms.get('pixelSize');
    if (uniform) uniform.value = v;
  }
  get pixelSize() {
    return this.retroUniforms.get('pixelSize')?.value as number;
  }
}

const WrappedRetro = wrapEffect(RetroEffectImpl) as unknown as ComponentType<{ colorNum: number; pixelSize: number }>;
...
function DitheredWaves({
  ...
  useFrame(({ clock }) => {
    ...
    if (prevWaveColor.current.some((value, index) => waveColor[index] !== value)) {
...
export default function Dither({
  ...
  waveFrequency = 3,
  waveAmplitude = 0.3,
  waveColor = [0.00392156862745098, 0, 1],
  colorNum = 4,
  pixelSize = 2,

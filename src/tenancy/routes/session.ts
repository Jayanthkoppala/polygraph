...
import { InvalidApiKeyFormatError, revealPlaintext } from '../secrets.js';
...
import { scopeWithSecrets, readJsonBody, requireCsrf, buildDashboardState } from './context.js';
import type { TenantRow } from './context.js';
import { MAX_COLLECTORS_DEFAULT, MAX_CANARY_INPUTS, PROBE_LIMIT_PER_DAY } from './context.js';
/**
 * Routes that require a signed-in session. The caller has already resolved
 * `session` and `tenantRow`. Returns true when it answered the request.
 */
export async function handleSessionRoutes(ctx: RouteContext, session: Session, tenantRowWriter: TenantRow): Promise<boolean> {

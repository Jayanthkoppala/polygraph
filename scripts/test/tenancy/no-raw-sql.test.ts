 ...
 * src/http/server.ts and src/fixture/server.ts are allowlisted for exactly that
...
describe('src/tenancy/ isolation — no hand-written SQL outside it', () => {
  it('no module outside the persistence-owner allowlist prepares its own SQL (Layer 2, tenant-architecture.md §3)', () => {
    ...
    const offenders = (output.length > 0 ? output.split('\n') : [])
      ...
      .filter((f) => !['src/store/ledger.ts', 'src/loop/policy.ts', 'src/loop/alerts.ts', 'src/store/safe-output.ts'].includes(f))
      ...
      .filter((f) => !['src/http/server.ts', 'src/fixture/server.ts'].includes(f));
